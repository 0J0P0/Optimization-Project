...
function [Xtr,ytr,wo,fo,tr_acc,Xte,yte,te_acc,niter,tex] = uo_nn_solve(num_target,tr_freq,tr_seed,tr_p,te_seed,te_q,la,epsG,kmax,ils,ialmax,kmaxBLS,epsal,c1,c2,isd,sg_al0,sg_be,sg_ga,sg_emax,sg_ebest,sg_seed,icg,irc,nu)
    ...
    w = ones(size(Xtr,1),1);                                               %% como defines w?
    ...
    [wk,dk,Lk,gLk,alk,iWk,betak,Hk] = uo_nn_solve_opt(w,L,gL,epsG,kmax,ialmax,kmaxBLS,epsal,c1,c2,isd,icg,irc,nu);

    wo=wk(:,end);fo=Lk(:,end);tr_acc=0;te_acc=0;niter=0;tex=0;
end
% [end] Neural Network OM solve %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


% [start] Alg. %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [wk,dk,Lk,gLk,alk,ioutk,betak,Hk] = uo_nn_solve_opt(w,L,gL,epsG,kmax,ialmax,kmaxBLS,epsal,c1,c2,isd,icg,irc,nu)
    k = 1; n = size(w,1); DC = 1;
    wk = [w]; dk = []; d = -gL(w); Lk = [L(w)]; gLk = [gL(w)]; alk = []; ioutk = [];  %% d sobra
    betak = []; Hk = [];
